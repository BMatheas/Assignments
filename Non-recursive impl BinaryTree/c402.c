...
**/
	...
	bool found=FALSE;
	tBTNodePtr pos = NULL, tmp_ptr  = (*RootPtr);

	while((found == FALSE) && tmp_ptr )
	{
	    pos = tmp_ptr;
		if(tmp_ptr->Cont < Content)
	      	 tmp_ptr = tmp_ptr->RPtr;
	    else if(tmp_ptr->Cont > Content)
	      	tmp_ptr = tmp_ptr->LPtr;
	    else if(tmp_ptr->Cont == Content)
	      	found = TRUE;  
	}
...
**/
	...
	tStackP tmp_stack;
	SInitP(&tmp_stack);
	Leftmost_Preorder(RootPtr, &tmp_stack);
	while(!SEmptyP(&tmp_stack))
	{
		RootPtr = STopPopP(&tmp_stack);
		Leftmost_Preorder(RootPtr->RPtr, &tmp_stack);	
	}
}
...
**/
	...
	tStackP tmp_stack;
	SInitP(&tmp_stack);
	Leftmost_Inorder(RootPtr, &tmp_stack);
	while(!SEmptyP(&tmp_stack))
	{
		RootPtr = STopPopP(&tmp_stack);
		BTWorkOut(RootPtr);
		Leftmost_Inorder(RootPtr->RPtr, &tmp_stack);
	}
}
...
**/
	...
	Leftmost_Postorder(RootPtr, &tmp_stackP, &tmp_stackB);

	while(!SEmptyP(&tmp_stackP))
	{
		RootPtr = STopPopP(&tmp_stackP);
		SPushP(&tmp_stackP, RootPtr);
		left = STopPopB(&tmp_stackB);
		if(left)
		{
			SPushB(&tmp_stackB, FALSE);
			Leftmost_Postorder(RootPtr->RPtr, &tmp_stackP, &tmp_stackB);
		}
		else
		{
			STopPopP(&tmp_stackP);
