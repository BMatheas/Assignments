...
*/
	if(queueEmpty(q)) //kontrola ci je prazdna fronta
	{
		queueError(QERR_FRONT); //nasledne volanie ERROR
		return;
	}
	*c = q->arr[q->f_index]; // vrati znak do premmenej c
}
...
*/
	if(queueEmpty(q)) //kontrola ci je prazdna fronta
	{
		queueError(QERR_REMOVE);//nasledne volanie ERROR
		return;
	}
	q->f_index = nextIndex(q->f_index); //zmazanie prvku 
}
...
*/
	if(queueEmpty(q))//kontrola ci je prazdna fronta
	{
		queueError(QERR_GET); //nasledne volanie ERROR
		return;
	}
	queueFront(q, c); //vlozi znak z fronty do premmenej c
	queueRemove(q); //zmaze znak
}
...
*/
	if(queueFull(q)) //kontrola ci je plna fronta
	{
			queueError(QERR_UP); // nasledne volanie ERROR
			return;
	}
	q->arr[q->b_index] = c; //vlozenie prvku do fronty
	q->b_index = nextIndex(q->b_index); //posun indexu na dalsi prazdny prvok
}
